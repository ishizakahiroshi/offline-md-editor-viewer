...
**完全オフラインで動作します。** `marked`、`DOMPurify`、`encoding-japanese`、`highlight.js` はインライン化して同梱しており、`Feather Icons` の SVG アイコンも HTML に埋め込み済みのため、アプリ読み込み時に CDN や外部サービスへの通信は発生しません。外部リンクは、設定の「外部リンク」を有効にして `http` / `https` リンクをクリックした場合だけブラウザで開きます。
...
- **すばやい移動と集中モード。** コマンドパレット（`Ctrl/Cmd+K`）、見出しアウトライン、集中執筆用の Zen モード、最近使ったファイル一覧、未保存変更の差分表示に対応。
- **書いた量がひと目で分かります。** ステータスバーに文字数・単語数・推定読了時間を表示し、ツールバーの現在ファイル名とフルパスはそれぞれワンクリックでコピーできます。
- **読みやすいデフォルト。** 見出し・表・引用・シンタックスハイライト付きコードブロックに調整したダーク/ブルー/ライトテーマ、文字サイズ変更にも対応。
...
- **デスクトップ版のファイルアクセスは範囲を限定します（Windows）。** デスクトップ版は、開く・保存する・関連付けで起動する・ドロップして認可したフォルダの中だけを読み書きします。その外のパスは Rust 側で拒否します。このバージョンへ更新した直後は、フォルダを一度開き直すと再び認可されます。
...
編集モードでは `Ctrl/Cmd+F` でフローティング検索バーを開き、`Ctrl/Cmd+H` または `Ctrl/Cmd+R` で置換操作まで展開できます。`Enter` で次の一致、`Shift+Enter` で前の一致へ移動し、`Esc` で閉じます。大小文字区別、単語単位、正規表現検索に対応しています。なお正規表現置換では、前後の文脈に依存する後読み/先読み（例: `(?<=foo)bar`）は、置換が一致した部分文字列ごとに適用される都合上、正しく置換されない場合があります。

`Ctrl/Cmd+K`（または `Ctrl/Cmd+Shift+P`）でコマンドパレットを開き、主要な操作をキーボードから実行できます。Zen モードは周囲のパネルを隠して執筆に集中でき、`Esc` で終了します。
開いている文書はエディタ上部にタブとして並びます。タブをクリックすると切り替わります。本文・アンドゥ履歴・カーソル位置・文字コード・BOM・改行コード・読み取り専用は文書ごとに保持されるので、タブを切り替えても別の文書の設定が混ざることはありません。`+` は無題のタブを新しく開きます。`新規` はフォルダを開いているときはそのフォルダ内にファイルを作り、開いていないときは無題のタブを開きます。どちらの場合も、いま編集中の内容を置き換えることはなくなりました。未保存のタブには印が付き、閉じるときに「保存して閉じる／破棄／戻る」を確認します。ウィンドウを閉じるときも未保存の文書を 1 件ずつ同じ形で確認し、どれか 1 つでも取り消すと終了しません。

タブ、またはタブ列の空きを右クリックすると、新しいタブ・閉じる系（このタブ／他のタブ／右側のタブ）・コピー系（ファイル名／パス）のメニューが出ます。Desktop 版では、ファイルの場所をエクスプローラーで開く項目も出ます。
...
単体 HTML は完全な self-contained ファイルです。`marked`・`DOMPurify`・`encoding-japanese`・`highlight.js`・`Feather Icons` の SVG セットがすべてインライン化されており、余分なフォルダは不要で、ファイルを開くだけでオフライン動作します。ライセンス全文は About ダイアログ → **ライセンス本文を表示** からいつでも確認できます。
...
exe はポータブルです。インストール不要で、USB メモリからも直接実行できます。デスクトップ版はブラウザ版と同じ単一 HTML を Tauri の WebView で読み込むため、フロントエンドコードは両版で完全に共通です。About ダイアログ → **ライセンス本文を表示** から、Web 依存（marked / DOMPurify / encoding-japanese / highlight.js / Feather Icons）と exe に静的リンクされた Tauri/Rust クレートすべてのライセンス全文を確認できます。
...
macOS 版のデスクトップアプリを作る予定はありません。macOS ではブラウザ版をご利用ください。Linux 版は検討中です。それまではブラウザ版をご利用ください。
...
| フォルダのドラッグ&ドロップ | ✓ | ✓ |
| ツリー内でのファイル/フォルダの作成・削除 | ✓（Chrome 標準の File System Access API 利用時） | ✓ |
| ツリー内でのファイル名変更 | ✓（Chrome 標準の File System Access API 利用時） | ✓ |
| ツリー内でのフォルダ名変更 | | ✓ |
...
| エクスプローラーからファイルを開くと、別のコピーを起動せず実行中のウィンドウにタブが増える | | ✓ |

デスクトップ版では、アプリの起動中にエクスプローラーから別の `.md` を開くと、アプリがもう 1 つ起動するのではなく、既存のウィンドウにタブが増えてそのウィンドウが前面に出ます。すでに開いているファイルなら、そのタブが選択されるだけです。別のフォルダに置いたコピー同士は独立したままなので、USB メモリのポータブル版とインストール済みの版が 1 つのウィンドウにまとまってしまうことはありません。
...
- `フォルダ` を使うと、フォルダとファイルを左側のツリーに表示できます。ツリーは既定ですべての形式を表示し、バイナリファイルは開く時点で拒否します。
...
- ブラウザが対応している場合は、フォルダのドラッグ&ドロップでも中のファイルを表示できます。
...
- `marked`、`DOMPurify`、`encoding-japanese`、`highlight.js` はインライン化して同梱しており、`Feather Icons` の SVG アイコンも HTML に埋め込み済みのため、アプリ読み込み時に CDN への通信は発生しません。
