...
なお、HTML内定数ではバッジ画像（shields.io）・GitHub README 用スクリーンショット（`docs/assets/`）・GitHub Releases への Quick Download 導線・相互言語リンク行・mermaid図・外部リンク URL は除外している（オフライン表示のため）。それ以外の本文・セクション順序は4箇所すべてで同期を保つこと。
